...
Astra Trident 提供 `solidfire-san` 用於與叢集通訊的儲存驅動程式。支援的存取模式包括： _ReadWriteOnce_ （ rwo ）、 _ReadOnlyMany （ ROX ）、 _ReadWriteMany （ rwx ）、 _ReadWriteOncePod （ RWOP_ ）。

。 `solidfire-san` 儲存驅動程式支援 _file_ 和 _block_ 磁碟區模式。適用於 `Filesystem` 磁碟區代碼、Astra Trident會建立磁碟區並建立檔案系統。檔案系統類型由StorageClass指定。
