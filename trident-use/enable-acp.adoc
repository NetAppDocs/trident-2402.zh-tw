...
如果您為 Astra Control Service 新增一個先前未安裝 Astra Trident 的叢集，則該叢集將被標記為 `Eligible`。將叢集新增至 Astra Control 後，Astra Control Provisioner 將自動啟用。
...
. [[no-registry]]使用您最初使用的安裝方法在 Astra Trident 中啟用 Astra Control Provisioner：
+
[role="tabbed-block"]
====
.Astra Trident 運算子
--
.. https://docs.netapp.com/us-en/trident/trident-install/kubernetes-deploy-operator.html#step-1-download-the-trident-installer-package["下載 Astra Trident 安裝程式並將其解壓縮"^]。
...
----
+
... 設定 Astra Trident 映像的自訂登錄位置、或從 Astra Control 登錄或中拉出 (`tridentImage: <my_custom_registry>/trident:24.02.0` `tridentImage: netapp/trident:24.02.0`）。
...
.試用
--
.. https://docs.netapp.com/us-en/trident/trident-install/kubernetes-deploy-tridentctl.html#step-1-download-the-trident-installer-package["下載 Astra Trident 安裝程式並將其解壓縮"^]。
