...
是的，Astra Trident 可以離線安裝。請參閱 link:./trident-install/kubernetes-deploy.html["瞭解Astra Trident安裝"^]。
...
Trident 安裝程式會建立一個 Kubernetes ClusterRole，該實例擁有對 Kubernetes 叢集的 PersistentVolume、PersistentVolumeClaim、StorageClass 和 Secret 資源的特定存取權。請參閱 link:./trident-install/kubernetes-customize-deploy-tridentctl.html["自訂tridentctl安裝"^]。
...
如有需要，您可以本地產生和修改 Astra Trident 安裝時使用的確切清單檔案。請參閱 link:./trident-install/kubernetes-customize-deploy-tridentctl.html["自訂tridentctl安裝"^]。
