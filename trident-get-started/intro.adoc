...
若要進行試用，請申請存取「輕鬆部署和複製容器化工作負載的持久性儲存」解決方案link:https://www.netapp.com/test-drives/["NetApp試用"^]，該方案提供即用型實驗室映像。試用環境包含一個沙箱環境，其中安裝並設定了三節點 Kubernetes 叢集和 Astra Trident。這是熟悉 Astra Trident 並探索其功能的絕佳方式。
