...
Cloud Volumes ONTAP 是一款純軟體儲存設備，可在雲端運作 ONTAP 資料管理軟體。您可以使用 Cloud Volumes ONTAP 處理生產工作負載、災難復原、DevOps、檔案共用和資料庫管理。它透過提供儲存效率、高可用性、資料複製、資料分層和應用程式一致性，將企業儲存擴展到雲端。
...
https://azure.microsoft.com/en-us/products/netapp/["Azure NetApp Files"^] 是由 NetApp 提供支援的企業級 Azure 檔案共用服務。您可以在 Azure 中原生運作要求最嚴苛的基於檔案的工作負載，並獲得您期望從 NetApp 獲得的卓越效能和豐富的資料管理功能。
...
https://www.netapp.com/google-cloud/netapp-volumes/["適用於Cloud Volumes Service Google Cloud的NetApp解決方案"^] 是一項雲端原生檔案服務，可透過 NFS 和 SMB 提供 All Flash 效能的 NAS 磁碟區。該服務支援任何工作負載（包括傳統應用程式）在 GCP 雲端上運行。它提供完全代管服務，可實現一致的高效能、即時複製、資料保護，以及對 Google Compute Engine (GCE) 執行個體的安全存取。
