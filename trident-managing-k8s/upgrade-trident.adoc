...
* 如果您要從超出四版本視窗期的版本升級，請執行多步驟升級。使用您要升級的早期版本的升級說明，升級到符合四版本視窗期的最新版本。例如，如果您目前運行的是 22.01 版本，並且想要升級到 24.02 版本：
+
...
通常情況下，您應該使用與初始安裝相同的方法進行升級，但您也可以link:../trident-install/kubernetes-deploy.html#moving-between-installation-methods["在安裝方法之間移動"]。升級 Astra Trident 有兩種選項。
