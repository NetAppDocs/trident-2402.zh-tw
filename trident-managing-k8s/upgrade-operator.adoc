...
[role="lead"]
您可以使用 Trident operator 手動或透過 Helm 升級 Astra Trident。您可以從一個 Trident operator 安裝升級到另一個 Trident operator 安裝，也可以從 `tridentctl`安裝升級到 Trident operator 版本。在升級 Trident operator 安裝之前，請先檢閱link:upgrade-trident.html#step-3-select-an-upgrade-method["選擇升級方法"]。
...
.步驟
. 如果您link:../trident-install/kubernetes-deploy-helm.html#deploy-the-trident-operator-and-install-astra-trident-using-helm["使用 Helm 安裝 Astra Trident"]，可以使用 `helm upgrade trident netapp-trident/trident-operator --version 100.2402.0`一步升級。如果您沒有新增 Helm 倉庫或無法使用它進行升級：
+
...
NOTE: 在切換安裝方法之前，請先查看 link:../trident-install/kubernetes-deploy.html#moving-between-installation-methods["在安裝方法之間移動"]。
