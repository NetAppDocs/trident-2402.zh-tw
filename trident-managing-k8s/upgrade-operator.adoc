...
.步驟
. 如果您link:../trident-install/kubernetes-deploy-helm.html#deploy-the-trident-operator-and-install-trident-using-helm["使用 Helm 安裝 Astra Trident"]，可以使用 `helm upgrade trident netapp-trident/trident-operator --version 100.2402.0`一步升級。如果您沒有新增 Helm 倉庫或無法使用它進行升級：
+
