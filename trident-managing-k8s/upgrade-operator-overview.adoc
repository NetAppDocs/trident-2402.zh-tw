...
安裝和升級 Astra Trident 的眾多link:../trident-install/kubernetes-deploy.html["使用 Trident 運算子的優點"]之一是能夠自動處理 Astra Trident 和 Kubernetes 物件，而不會中斷現有掛載的磁碟區。如此一來，Astra Trident 即可支援零停機升級，或link:https://kubernetes.io/docs/tutorials/kubernetes-basics/update/update-intro/["滾動更新 _"^]。具體而言，Trident operator 與 Kubernetes 叢集通訊以：
...
TIP: 有關 Kubernetes 叢集上 Astra Trident 架構的詳細資訊，請參閱link:../trident-get-started/architecture.html["Astra Trident 架構"]。
