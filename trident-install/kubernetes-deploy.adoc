...
* 對執行受支援 Kubernetes 版本且已啟用相關功能需求的受支援 Kubernetes 叢集擁有完整權限。詳情請查看 link:../trident-get-started/requirements.html["需求"]。
* 存取支援的NetApp儲存系統。
...
選擇適合您的安裝方式。在做出決定之前，您還應該考慮 link:kubernetes-deploy.html#moving-between-installation-methods["在方法之間移動"]。
...
無論手動部署或使用 Helm，Trident operator 都是簡化安裝和動態管理 Astra Trident 資源的絕佳方式。您甚至可以link:kubernetes-customize-deploy.html["自訂您的Trident營運者部署"]使用 `TridentOrchestrator`自訂資源 （CR） 中的屬性。
